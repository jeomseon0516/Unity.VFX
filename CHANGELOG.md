...
- 사용자에게 Unity `GameObject`/`MonoBehaviour`를 반환하지 않고 generation을 검증하는
  ...
  종료하거나 조작할 수 없습니다.
- Particle 완료·수동·시간(Scaled/Unscaled) 수명 Configuration·Handler·Session과
  사용자 `IVFXLifetimeHandler` 등록 경로를 추가했습니다.
...
- `VFXInstance`의 수명 Tick 루프를 `Coroutine`에서 `Awaitable`
  ...
  GameObject 비활성 중에도 계속 진행되므로 그렇게 오용됐을 때 `ParticleCompletionVFXLifetimeHandler`가
